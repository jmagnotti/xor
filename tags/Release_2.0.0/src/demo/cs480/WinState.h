...
/**
 * In the WinState, collision detection is off and the ball is "bound" to the 
...
public:
	...
	/**
	 * Returns a pointer to the WinState singleton. This method
	 ...
	 */
	static WinState * GetInstance(Game * game);
...
private:
	/*
	 * Default singleton constructor. If there is no WinState already
	 * created, it instantiates one and returns a pointer to that instance.
	 * If a WinState is already instantiated, it returns a pointer to the
	 * current instance. The constructor is ONLY called by the getInstance method
	 */
	WinState();


	static WinState * _state;
};
#endif //WINSTATE_H
