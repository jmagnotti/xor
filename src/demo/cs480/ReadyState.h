...
/**
 * In the ReadyState, collision detection is off and the ball is "bound" to the 
...
public:
	...
	/**
	 * Returns a pointer to the ReadyState singleton. This method
	 ...
	 */
	static ReadyState * GetInstance(Game * game);
...
private:
	/*
	 * Default singleton constructor. If there is no ReadyState already
	 * created, it instantiates one and returns a pointer to that instance.
	 * If a ReadyState is already instantiated, it returns a pointer to the
	 * current instance. The constructor is ONLY called by the getInstance method
	 */
	ReadyState();


	static ReadyState * _state;
};
