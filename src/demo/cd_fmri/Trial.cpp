...
void Trial::recordChoice(string outputFile, int location, int responseTime)
{
    ...
    //--newly added for response encoding
			<< "choiceLocation='" << location << "' "
			<< "responseTime='" << responseTime << "' ";
    if (location > 0) 
        fout << "choiceLabel='" << location << "' ";
    else if (location == -99)
        fout << "choiceLabel='no-choice' ";
    else 
        fout << "choiceLabel='" << (location==CHOOSE_CHANGE? "change" : "no-change") << "' ";
    //--

    fout    << "sampleDisplaySize='" << _sampleDisplaySize << "' "
			...
			<< "decideDuration='" << _decideDuration << "' "
			<< "choiceDuration ='" << _choiceDuration<< "' "
...
Trial::Trial(XMLNode trialNode)
{
	...
	_decideDuration = atoi(trialNode.getAttribute("decideDuration"));
	_choiceDuration = atoi(trialNode.getAttribute("choiceDuration"));
...
Trial::Trial(int ttype, int sds, int cds, int fd, int sdd, int retIntv, int decideDuration, int choiceDuration, int iti, string fixationImageFile, vector<string> sFiles, vector<int> sLocs, vector<string> cFiles, vector<int> cLocs)
{
	_trialType = ttype;
    ...
    _decideDuration = decideDuration;
...
void Trial::printXMLToFile(ofstream * fout)
{
	*fout 	<< "\t<Trial trialNumber='" << _trialNumber << "' trialType='" << _trialType << "' "
			<< "sampleDisplaySize='" << _sampleDisplaySize << "' "
			<< "choiceDisplaySize='" << _choiceDisplaySize << "' "
...
int Trial::getDecideDuration()
{
    return _decideDuration;
}
