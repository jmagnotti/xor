...
{
    if (_currentTrial == 0) return _trials[_currentTrial]->getInterTrialInterval();

	return _trials[_currentTrial-1]->getInterTrialInterval();
}

int Session::getDecideDuration()
{
	return _trials[_currentTrial-1]->getDecideDuration();
}

int Session::getChoiceDuration()
{
    return _trials[_currentTrial-1]->getChoiceDuration();
}
