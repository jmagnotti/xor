...
public:
     ...
     * int: decide (choice array display) duration
     * int: choice (question mark display) duration
	 ...
	 */
	Trial(int, int, int, int, int, int, int, int, int, string, vector<string>, vector<int>, vector<string>, vector<int>);
	...
	int getDecideDuration();
	int getChoiceDuration();
    ...
    /**
     * Outputs XML to the specified ofstream
     */
    void printXMLToFile(ofstream * fout);
...
private:
	...
	int _fixationDuration, _sampleDisplayDuration, _retentionInterval, 
	    _decideDuration, _choiceDuration, _interTrialInterval;
