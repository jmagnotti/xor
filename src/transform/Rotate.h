...
public:
    ...
    //ImmediateRotates. Should the Action be fired after first push? If so, the
    //action will occur before any change is seen -- this may not be the
    //expected behavior, as the user may want something to happen sequentially.
	 ...
	 */
    static Rotate * CreateRotate(float angle, const Vector3D * axis, 
            int milliseconds, const Vector3D * center=ORIGIN);

    static Rotate * CreateRotate(float angle, const Vector3D * axis, 
            int milliseconds, Action * action, const Vector3D * center=ORIGIN);
    ...
    /**
     * removes the rotation. needed so we can pop the centering translation
     */
	void pop(void);
...
public:
	...
	InterpolatedRotate(float angle, const Vector3D * axis, 
...
public:
	...
	ImmediateRotate(float angle, const Vector3D * axis, const Vector3D * center);
	ImmediateRotate(float angle, float x, float y, float z, const Vector3D * center);
