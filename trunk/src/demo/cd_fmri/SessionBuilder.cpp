...
void SessionBuilder::build(const char * xmlFile)
{
    ...
    vector<int> itiDistribution;
    vector<int> sampleDistribution;
    vector<int> delayDistribution;
    vector<int> decideDistribution;
    vector<int> choiceDistribution;

    for(int j=1; j<=2; j++) {
        for (int i=0; i<48/2; i++) {
            itiDistribution.push_back(j+2);
            sampleDistribution.push_back(j);
            delayDistribution.push_back(j);
            decideDistribution.push_back(j);
            choiceDistribution.push_back(j);
        }
    }

    Helper::Reorder(&itiDistribution);
    Helper::Reorder(&sampleDistribution);
    Helper::Reorder(&delayDistribution);
    Helper::Reorder(&decideDistribution);
    Helper::Reorder(&choiceDistribution);
    ...
    //break this out into another method just to keep this cleaner?
        ...
        for(int block=0; block<(nTrials/blockSize) / 2; block++) {
                cout << endl << "Starting block: " << block << endl;
            for (int trialType = 0; trialType < 2; trialType++) {
                for (int j=0; j<blockSize; j++) {
                    cout << (j + blockSize*trialType) << ".";
                    ...
                    interTrialInterval = itiDistribution[__counter];
                    sampleDuration = sampleDistribution[__counter];
                    retentionInterval = delayDistribution[__counter];
                    decideDuration = decideDistribution[__counter];
                    choiceDuration = choiceDistribution[__counter];

                    __counter++;

                    trials.push_back(new Trial(trialType, sds, cds,
                                                fixationDuration, sampleDuration, retentionInterval, decideDuration, choiceDuration, interTrialInterval,
        ...
        lbl = "Change/NoChange";
        fname = "cnc_temp.xml";
        reportingMethod = Session::METHOD_CHANGE_NOCHANGE;
        printSession(fname, lbl, seed, trials);

        lbl = "Forced-Choice";
        fname = "fc_temp.xml";
        reportingMethod = Session::METHOD_WHICH_CHANGED;
        printSession(fname, lbl, seed, FCtrials);

        lbl = "Same/Different";
        fname = "sd_temp.xml";
        reportingMethod = Session::METHOD_SAME_DIFF;
        printSession(fname, lbl, seed, SDtrials);

        lbl = "Forced-Choice_NoChange";
        fname = "ncfc_temp.xml";
        reportingMethod = Session::METHOD_CHOOSE_SAME;
        printSession(fname, lbl, seed, NCFCtrials);
...
void SessionBuilder::buildRandomLocationSession()
{
    ...
    for(int block=0; block < (nTrials/blockSize) ; block++) {
        for (int j=0; j<blockSize; j++) {
            ...
            cout << "Don't trust the build results." << endl;
            cout << "OH NO, something is wrong in denmark. Please see L346-355 in SessionBuilder.cpp" << endl;
            cout << "Don't trust the build results." << endl;

            decideDuration = sampleDuration;
            choiceDuration = decideDuration;
...
void SessionBuilder::buildRetrievalStroopSession()
{
    ...
    for(int block=0; block < (nTrials/blockSize) / 2 ; block++) {
        for (int trialType = 0; trialType < 2; trialType++) {
            for (int j=0; j<blockSize; j++) {
                ...
                cout << "Don't trust the build results." << endl;
                cout << "OH NO, something is wrong in denmark. Please see L475-480 in SessionBuilder.cpp" << endl;
                cout << "Don't trust the build results." << endl;
    ...
    string fname("retStroop_cnc.xml");
    printSession(fname, lbl, seed, trials);
}
...
void SessionBuilder::printSession(string fname, string label, int seed, vector<Trial*> trials)
{
    ofstream fout;
    fout.open(fname.c_str());
    
    fout << "<Session label='" << label <<"' id='" << seed
         << "' reportingMethod='" << reportingMethod << "'>" << endl;

    fout << "<Trials count='" << nTrials << "'>" << endl;

    for (int i=0; i<trials.size(); i++) {
        trials[i]->setTrialNumber(i+1);
        trials[i]->printXMLToFile(&fout);
    }
