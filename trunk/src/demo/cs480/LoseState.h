...
/**
 * In the LoseState, collision detection is off and the ball is "bound" to the 
...
public:
	...
	/**
	 * Returns a pointer to the LoseState singleton. This method
...
private:
	/*
	 * Default singleton constructor. If there is no LoseState already
	 * created, it instantiates one and returns a pointer to that instance.
	 * If a LoseState is already instantiated, it returns a pointer to the
	 * current instance. The constructor is ONLY called by the getInstance method
	 */
	LoseState();


	static LoseState * _state;
};
#endif //LOSESTATE_H
